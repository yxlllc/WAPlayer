...
(
	Quiet@JLink`InstallJava[JLink`JVMArguments->"-Xmx32g"];
...
MixPlayer[audio_,bytes_]:=
	DynamicModule[{sequencer,midistream,options,id,duration,midilength},
		...
		midilength=sequencer@getMicrosecondLength[];

	Row[{
		Button[
			buttonstyle["Play"],
		
			If[sequencer@getMicrosecondPosition[]==midilength,
		...
		Button[
			...
			sequencer@setMicrosecondPosition@Min[Round[10^6 Audio`AudioStreamInternalsDump`$audioStreams["Internal",id,"Position"]],midilength],
		...
		Slider[
			Dynamic[
				Min[Audio`AudioStreamInternalsDump`$audioStreams["Internal",id,"Position"],duration],
					...
					(sequencer@setMicrosecondPosition@Min[Round[10^6 #],midilength];
